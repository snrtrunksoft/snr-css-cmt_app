...
const AdminItems = () => {
    ...
    useEffect(() => {
        const fetchStoreItems = async () => {
            try {
                const response = await fetch("https://1p6bhnsms5.execute-api.us-east-1.amazonaws.com/dev/items");
                if (!response.ok) {
            ...
            if (existingItem) {
                if (existingItem.isUpdated === true) {
                    const updatedFields = async () => {
                        try {
                            fetch(`https://1p6bhnsms5.execute-api.us-east-1.amazonaws.com/dev/items/${idToSave}`, {
                                method: "PUT",
                                headers: {
    ...
    const handleDelete = (idToDelete) => {
        if (rowData.length >= 1) {
            const deleteField = async () => {
                try {
                    fetch(`https://1p6bhnsms5.execute-api.us-east-1.amazonaws.com/dev/items/${idToDelete}`, {
                        method: "DELETE",
                    })
                        .then(response => response.json());
                    setIsStatusModalOpen(true);
    ...
    const handleUpload = (itemId, file) => {
        ...
        s3.upload(params, (err, data) => {
            if (err) {
                ...
                handleFieldChange(itemId, 'image', `https://1p6bhnsms5.execute-api.us-east-1.amazonaws.com/dev/items/${file.name}`)
                setUploadButtonHide(true);
            }
        });
    };
    ...
    return (
        <>
            <Row gutter={0} justify="start" className="rowBox-top-item">
                ...
                <Col span={10} key="Item-name-header" flex="1 1 100px">
            ...
            {availableItems.map((row) => (
                <Row key={row.id} gutter={0} justify="start">
                    <Col span={2} style={{textOverflow:'ellipsis',whiteSpace:'nowrap',overflow:'hidden',textAlign:'left'}}
                     key="category-id" flex="1 1 100px">
