      ...
      if (existingItem) {
        if (existingItem.isUpdated === true) {
          const fetchUpdatedField = async () => {
            try {
            fetch(`https://l2wifnx033.execute-api.us-east-2.amazonaws.com/categories/${idToSave}`,{
              method: "PUT",
              headers: {
  ...
  const handleDelete = (idToDelete) => {
    if (rowData.length > 0) {
      const deleteCategory = async () => {
        try {
          fetch(`https://l2wifnx033.execute-api.us-east-2.amazonaws.com/categories/${idToDelete}`,{
            method:'DELETE',
          })
