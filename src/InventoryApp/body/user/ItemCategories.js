...
import { Row, Col, Button, Divider, Checkbox, Select } from "antd";
...
const ItemCategories = ({ selectedFilter, setSelectedFilter, setFilteredItems, availableItems, searchText, setSearchText }) => {
    ...
    const fetchItemCategories = async () => {
      try {
        const response = await fetch(
          " https://cx8712tt7e.execute-api.us-east-1.amazonaws.com/dev/categories"
  ...
  console.log(groupedCategories.map((item) => (item.map(zz => zz.name))));
    ...
    <div>
      <Row>
        ...
        <Select
          ...
          onChange={(value) => {setSelectedItemCategory(value);setSelectionCriteria("categories")}}>
          {groupedCategories.map((itemCategory) => (
            <>
              {itemCategory.map((dropDownCategories) => (
                <Option
                    type="default"
                    value={dropDownCategories.name.toLocaleLowerCase()}
                    style={{
                      textAlign: "center",
                      backgroundColor:
                        selectedItemCategory === dropDownCategories.name.toLocaleLowerCase() ? "#0582f7" : "inherit",
                      color:
                        selectedItemCategory === dropDownCategories.name.toLocaleLowerCase() ? "#fff" : "inherit",
                      borderColor:
                        selectedItemCategory === dropDownCategories.name.toLocaleLowerCase()
                          ? "#52c41a"
                          : "inherit",
                    }}
                  >
                    {dropDownCategories.name}
                  </Option>
              ))}
            </>
            ))}
