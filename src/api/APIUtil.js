...
export async function getCalendar(entityId, userId, month, year) {
  ...
  const path = `/calendar/user/${userId}/month/${monthName}/year/${year}`;
  const op = get({
    apiName: "CalendarAPI",
    path,
    options: { headers: { entityid: entityId } },
...
export async function getCountries() {
  try {
    const url = (awsExports && awsExports.API && awsExports.API.ExternalApis && awsExports.API.ExternalApis.restCountries)
      || "https://restcountries.com/v3.1/all?fields=name,cca2,idd,region,states";
...
export async function getCountryStates(countryName) {
  try {
    const url = (awsExports && awsExports.API && awsExports.API.ExternalApis && awsExports.API.ExternalApis.countriesNowStates)
